                        ...
                        <table id="example5" class="table table-striped patient-list mb-4 dataTablesCard fs-14">
                            ...
                            @if(count($data)!="")
                                @foreach($data as $key=>$row)
                                    <tr>
                                        <td>{!! $key+1 !!}</td>
                                        <td><img src="{{asset($row->image)}}" alt="" width="60px"></td>
                                        <td>{!! $row->subject !!}</td>
                                    ...
                                    <span class="badge {!! $row->is_active == '1' ? 'btn-primary' : 'btn-danger' !!}">
                                        ...
                                        <td>
                                            <a href="{!! url('admin/update-subject?id='.$row->id) !!}">
            ...
            $.confirm({
                ...
                buttons: {
                    Confirm: {
                        ...
                        action: function(){
                            $.post("{!! url('admin/update-status') !!}", {'where[id]': id, 'input[is_active]': status, 'tab': "subject", '_token': "{!! csrf_token() !!}"}, function(html){
